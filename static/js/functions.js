...
function carritoAcciones(itemId,url,accion) {
  ...
  enviar_datos(url, datos, function (data) {//funcion que realiza acciones despues de la petición
      if (data["error"]) {
        //****************** que hubo un error
        const Toast = Swal.mixin({
          toast: true,
        ...
        if (
          ...
          if (data["check_total"] == 0) {
            ...
            const contenedorABorrar =
              document.getElementById("contenedor_todo");
